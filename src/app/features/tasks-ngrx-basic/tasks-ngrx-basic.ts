...
import { TaskSignalStoreBasic } from '../../core/task-signal-store-basic';
import { Priority, TaskFilter } from '../../core/models';
import { TaskItem } from '../tasks/task-item';

/**
 * Backed by the BASIC SignalStore (`withState` + `patchState`, plain array).
 * Compare with `/tasks-ngrx`, which upgrades the same store to
 * `withEntities` + `rxMethod`. Both kept intentionally for comparison.
 */
@Component({
  selector: 'app-tasks-ngrx-basic',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [ReactiveFormsModule, TaskItem],
  template: `
    <section class="page">
      <h1>Tasks <span class="tag">SignalStore · basic</span></h1>
      <p class="lead">
        <code>signalStore()</code> with plain <code>withState</code> holding a
        <code>Task[]</code>, mutated via <code>patchState</code>. Simplest
        SignalStore form — no entities, no RxJS.
      </p>
      ...
      <div class="filters">
        @for (f of filters; track f) {
          <button type="button" [class.active]="store.filter() === f" (click)="store.setFilter(f)">
            {{ f }}
          </button>
        }
      </div>
      ...
      <ul class="list">
        @for (task of store.visibleTasks(); track task.id) {
          <app-task-item [task]="task" (toggle)="store.toggle($event)" (remove)="store.remove($event)" />
        } @empty {
  ...
  styles: `
    ...
    .tag { font-size: 0.7rem; background: #6b7280; color: #fff; padding: 2px 8px; border-radius: 999px; vertical-align: middle; }
    .lead { color: #6b7280; }
...
export class TasksNgrxBasic {
  protected readonly store = inject(TaskSignalStoreBasic);
